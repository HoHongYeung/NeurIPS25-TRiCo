...
def run(config):
    ...
    num_classes = dconfig.get('num_classes')
    if num_classes is not None:
        config.num_classes = num_classes
    else:
        num_classes = getattr(config, 'num_classes', None)
    if config.model == TRICO_MODEL_KEY:
        if num_classes is None:
            raise ValueError("TRiCo requires 'num_classes' from dataset configuration.")
        loaders = create_loaders_v2(**dconfig, config=config)
        trainer = TRiCo.Trainer(device, config, num_classes)
        trainer.loop(config.epochs, *loaders, scheduler=None)
        return
    if config.model[-1]=='1':
