...
class SimCLRBackbone(nn.Module):
    """Frozen SimCLR ResNet-based encoder with classification head removed."""
    ...
    def __init__(self, pretrained: bool = True):
        ...
        try:
            ...
            encoder = torch.hub.load(
                "facebookresearch/simclr:main",
                "resnet50",
                pretrained=pretrained,
            )
        except Exception as exc:  # pragma: no cover
            print("[Warning] SimCLR not found, using ResNet50 fallback:", exc)
            try:
